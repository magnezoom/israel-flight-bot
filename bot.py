...
# key: (display name, API code for travelpayouts, booking city)
...
def start(update: Update, context: CallbackContext):
    update.message.reply_text(
        ...
        "Показываю реальные цены прямо в чате в шекелях ₪\n\n"
        "Что хотите найти?",
...
def button(update: Update, context: CallbackContext):
    ...
    if query.data in ("flights", "both"):
        ...
        context.user_data.update({"dest_key": dest_key, "dest_api": info[1],
                                   "dest_name": info[0], "dest_city": info[2]})
        query.edit_message_text(f"🔍 Ищу билеты → {info[0]}...\nПодождите!")
...
def _ask_checkout(query, checkin):
    ...
    query.edit_message_text(f"Заезд: {checkin} ✅\n\nСколько ночей?",
                            reply_markup=InlineKeyboardMarkup(keyboard))
...
def _send_hotel_links(query, context):
    ...
    text = (f"🏨 *Отели в {city_name}*\n"
            ...
            f"💡 Уже с вашими датами и ценами в ₪")
...
def _search_api(origin, destination):
    try:
        r = requests.get(
            "https://api.travelpayouts.com/v2/prices/latest",
            params={"origin": origin, "destination": destination,
                    "currency": "ils", "period_type": "month",
                    "one_way": "false", "token": TP_TOKEN},
            timeout=10
        )
        data = r.json().get("data", [])
        data.sort(key=lambda x: x.get("value", 9999999))
        return data
...
def _do_flight_search(query, context):
    ...
    results  = _search_api(origin, dest_api)
    ...
    if results:
        ...
        text = f"✈️ *Билеты {origin} → {dest_name}*\n\n"
        for i, r in enumerate(results[:5], 1):
            price   = r.get("value", "?")
            depart  = r.get("depart_date", "?")
            ret     = r.get("return_date", "")
            airline = r.get("gate", "")
            stops   = "прямой" if r.get("number_of_changes", 1) == 0 else f"{r.get('number_of_changes')} пересадка"
            ret_txt = f" | ↩️ {ret}" if ret else ""
            text += f"{i}. 📅 {depart}{ret_txt}\n   💰 *{price}₪*  •  {airline}  •  {stops}\n\n"
        text += f"🛒 [Купить на Aviasales]({aviasales})  |  [Google Flights]({gflights})"
    else:
        text = (f"✈️ *{origin} → {dest_name}*\n\n"
                f"Кешированных цен пока нет — смотрите актуальные:\n\n"
    ...
    if mode == "both" and dest_key in POPULAR_DESTINATIONS:
        query.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"🏨 Да, найти отели", callback_data=f"also_hotel_{dest_key}")],
...
def get_destination_manual(update: Update, context: CallbackContext):
    ...
    context.user_data.update({"dest_api": dest, "dest_name": dest, "dest_city": dest.lower()})
    origin = context.user_data.get("origin", "TLV")
    update.message.reply_text(f"🔍 Ищу {origin} → {dest}...")
    results  = _search_api(origin, dest)
    ...
    if results:
        ...
        for i, r in enumerate(results[:5], 1):
            ...
            airline = r.get("gate",""); stops = "прямой" if r.get("number_of_changes",1)==0 else f"{r.get('number_of_changes')} пересадка"
            text += f"{i}. 📅 {depart} | 💰 *{price}₪* | {airline} ({stops})\n\n"
...
def get_hotel_checkout_manual(update: Update, context: CallbackContext):
    ...
    city = context.user_data.get("hotel_city",""); city_name = context.user_data.get("hotel_city_name", city)
    checkin = context.user_data.get("hotel_checkin",""); checkout = context.user_data.get("hotel_checkout","")
    booking = f"https://www.booking.com/search.html?ss={city}&checkin={checkin}&checkout={checkout}&lang=ru&selected_currency=ILS&order=price"
...
def help_cmd(update: Update, context: CallbackContext):
    update.message.reply_text(
        ...
        "3. Бот покажет реальные цены прямо в чате\n"
